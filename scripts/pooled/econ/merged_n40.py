#!/usr/bin/env python3
"""Blind seed-expansion merge (PRIMARY.md Amendment 2): n=24 -> n=40 (10 seeds).
...
def onenas_run_series(s, sd):
    d = (f"probe1/island_champions/{s}_seed{sd}" if sd <= 44 else (f"probe_s4547/{s}_seed{sd}" if sd <= 47 else f"probe_s4851/{s}_seed{sd}"))
    ret, ic = {}, {}
...
print("== merged n=40 stats", flush=True)
...
mean10 = np.array([[onenas_pooled[sd][d] for sd in onenas_pooled] for d in common]).mean(1)
dts = [d for d in common if d in FF]
y = np.array([dict(zip(common, mean10))[d] for d in dts])
...
for l in range(1, lags + 1):
    ...
    S += w * (G + G.T)
V = XtXi @ S @ XtXi
OUT["alpha_n10"] = {"ann_alpha": round(float(beta[0]*252*100), 1),
                   "nw_t": round(float(beta[0]/math.sqrt(V[0,0])), 2),
                   "beta_mkt": round(float(beta[1]), 2)}
print(f"== 10-seed factor alpha: {OUT['alpha_n10']}", flush=True)
...
for sd in SEEDS:
    ...
    for s in SETS:
        d = (f"onenas_c7e/{s}_seed{sd}/ensemble_stitched_predictions.csv" if sd <= 44
             else (f"probe_s4547/{s}_seed{sd}/ensemble_stitched_predictions.csv" if sd <= 47
                   else f"probe_s4851/{s}_seed{sd}/ensemble_stitched_predictions.csv"))
        panel = PAN[s]
...
json.dump(OUT, open("results_econ/merged_n40.json", "w"), indent=1)
print("WROTE results_econ/merged_n40.json")

# ---- Amendment 2B: out-of-sample test of the aggregation law, IC(80) predicted +0.0132
print("== Amendment 2B: 80-champion super-ensemble", flush=True)
def rank01(v):
    n=len(v); r=np.empty(n); r[np.argsort(v,kind="stable")]=np.arange(n,dtype=float); return r/(n-1)
ics80=[]; rets80={}
for s in SETS:
    panel=PAN[s]; by={}
    for sd in SEEDS:
        ...
        pr,_=load_preds(d,panel,"2020-01-01","2024-12-31"); by[sd]=pr
    common=sorted(set.intersection(*[set(v) for v in by.values()]))
    ens={r:np.mean([rank01(by[sd][r]) for sd in SEEDS],axis=0) for r in common}
    days=scoring.build_days(panel,ens,common)
    bk=ss.run_book(days,scoring.PRED,panel.prc,panel.tc,10,None,book="sleeves",hold_days=10)
    ics80.append(np.mean([x for x in ss.daily_ics(days,scoring.PRED,ss.spearman) if x==x]))
    rets80[s]=dict(zip([d[1] for d in days],bk["daily_ret"]))
c=sorted(set.intersection(*[set(v) for v in rets80.values()]))
pool80=np.array([[rets80[s][d] for s in SETS] for d in c]).mean(1)
ic80=float(np.mean(ics80))
OUT["superensemble80"]={"ic":round(ic80,4),"predicted_ic":0.0132,
    "net":round(float(100*pool80.sum()),1),
    "sharpe":round(float(pool80.mean()/pool80.std(ddof=1)*math.sqrt(252)),2)}
print(f"  IC(80) measured {ic80:+.4f} vs predicted +0.0132  |  net {100*pool80.sum():+.1f}  Sharpe {pool80.mean()/pool80.std(ddof=1)*math.sqrt(252):.2f}", flush=True)
json.dump(OUT, open("results_econ/merged_n40.json","w"), indent=1)
print("WROTE results_econ/merged_n40.json (with 2B test)")
