#!/bin/sh
# Catastrophic Forgetting LSTM Baseline Experiment
#
# This script runs a fixed-architecture online LSTM under the exact same
# experimental protocol as catastrophic_forgetting.sh, to provide a baseline
# for comparison with ONE-NAS.
#
# The LSTM is continuously trained online on the sliding window of episodes,
# retaining weights across generations (standard online learning).
#
# Usage:
#   cd build && cmake .. && make catastrophic_forgetting_lstm_mpi
#   cd .. && bash scripts/experiments/catastrophic_forgetting_lstm_baseline.sh
...
exp_name="../results/catastrophic_forgetting_lstm_baseline/$i"
mkdir -p $exp_name
