...
# Submit with: sbatch scripts/experiments/catastrophic_forgetting_lstm_control.sh

#SBATCH -J cf_lstm_sliding
#SBATCH -A cis251123
#SBATCH -o cf_lstm_sliding_%A_%a.output
#SBATCH -e cf_lstm_sliding_%A_%a.error
...
exp_name="$ONENAS/results/catastrophic_forgetting_lstm_control/$TRIAL"
mkdir -p "$exp_name"
echo "Running LSTM Control Experiment (trial $TRIAL) - SlidingWindow (no replay)"
